...
export default function Documents(){
 ...
 return <><PageTitle eyebrow="Ressources" title="Documents" text="Documents internes accessibles uniquement aux amicalistes connectés." /><div className="privacy-note">🔒 Les fichiers sont stockés dans un espace privé. Les liens de consultation expirent automatiquement.</div>{loading?<div className="skeleton-card tall"/>:<div className="document-list">{items.length?items.map(x=><article className="document-row" key={x.id}><div className="file-icon">PDF</div><div><h3>{x.title}</h3><p>{x.description||'Document de l’amicale'}</p><small>{new Date(x.created_at).toLocaleDateString('fr-FR')}</small></div><button className="secondary-button" onClick={()=>openDocument(x)} disabled={opening===x.id}>{opening===x.id?'Ouverture…':'Consulter'}</button></article>):<div className="empty-state">Aucun document partagé.</div>}</div>}</>
}
